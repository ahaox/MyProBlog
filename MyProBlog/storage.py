...
class WatermarkStorage(FileSystemStorage):
    def save(self, name, content, max_length=None):
        ...
        if 'image' in content.content_type:
            # 加水印
            image = self.watermark_with_text(content, 'https://www.ahaoao.top', 'red')
    ...
    def watermark_with_text(self, file_obj, text, color, fontfamily='MyProBlog/themes/bootstrap/static/fonts/Dekko-Regular.ttf'):
        image = Image.open(file_obj).convert('RGBA')
        draw = ImageDraw.Draw(image)
        width, height = image.size
        margin = 10
        if fontfamily:
            font = ImageFont.truetype(fontfamily, size=25)
        else:
            font = None
